...
# Monthly income — computed directly from income DataFrame (always reliable)
_mi = float(inc['Monthly_Income'].dropna().iloc[-1]) if not inc['Monthly_Income'].dropna().empty else 0.0

# Affordability — compute directly, never depend on master column
_aff_d = round(_last['Dublin']     / _mi * 100, 1) if _mi > 0 else 0.0
_aff_n = round(_last['Non_Dublin'] / _mi * 100, 1) if _mi > 0 else 0.0

# Build affordability series by merging rent + income directly
_afd_merged = pd.merge(
    master[['Date','Dublin','Non_Dublin','Year']],
    inc[['Year','Monthly_Income']],
    on='Year', how='inner'
)
_afd_merged = _afd_merged[_afd_merged['Monthly_Income'] > 0].copy()
_afd_merged['Afford_Dublin']    = (_afd_merged['Dublin']     / _afd_merged['Monthly_Income'] * 100).round(2)
_afd_merged['Afford_NonDublin'] = (_afd_merged['Non_Dublin'] / _afd_merged['Monthly_Income'] * 100).round(2)
_afd_df = _afd_merged.sort_values('Date').reset_index(drop=True)
