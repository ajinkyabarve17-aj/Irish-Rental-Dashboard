...
def load_data():
    ...
    # Master dataset — built from rtb + exog data (exactly mirrors working original script)
    ...
    # Merge income on Year (left join), then ffill+bfill so ALL quarters get a value
    master = pd.merge(master, inc[['Year','Monthly_Income']], on='Year', how='left')
    master = master.sort_values('Date').reset_index(drop=True)
    master['Monthly_Income'] = master['Monthly_Income'].ffill().bfill()
    # YoY growth
    master['Dublin_YoY']    = master['Dublin'].pct_change(4) * 100
    master['NonDublin_YoY'] = master['Non_Dublin'].pct_change(4) * 100
    # Affordability — computed directly, no np.where needed since ffill guarantees values
    master['Afford_Dublin']    = (master['Dublin']     / master['Monthly_Income'] * 100).round(2)
    master['Afford_NonDublin'] = (master['Non_Dublin'] / master['Monthly_Income'] * 100).round(2)
...
# ── Pre-compute KPI values from full master ──────────────────────────────────
_last  = master.dropna(subset=['Dublin']).iloc[-1]
_first = master.dropna(subset=['Dublin']).iloc[0]
_unemp = master['Unemp'].dropna().iloc[-1] if not master['Unemp'].dropna().empty else 0.0
_mi    = float(master['Monthly_Income'].dropna().iloc[-1]) if not master['Monthly_Income'].dropna().empty else 0.0
_aff_d = round(float(_last['Dublin'])     / _mi * 100, 1) if _mi > 0 else 0.0
_aff_n = round(float(_last['Non_Dublin']) / _mi * 100, 1) if _mi > 0 else 0.0
# Affordability df — all rows where income is available
_afd_df = master.dropna(subset=['Afford_Dublin']).copy()
