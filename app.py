...
def safe_vline(fig, x, label, color, dash="dash", width=1.2, yref="paper"):
    """Compatible add_vline replacement — works on all Plotly versions."""
    fig.add_shape(type="line", x0=x, x1=x, y0=0, y1=1,
                  xref="x", yref=yref,
                  line=dict(color=color, width=width, dash=dash))
    fig.add_annotation(x=x, y=0.97, xref="x", yref=yref,
...
if page=="📊 Overview":
    ...
    # Always use master (full dataset) for KPI cards - never mf which may miss income data
    _kpi_latest = master.iloc[-1]
    _kpi_first  = master.iloc[0]
    _afford_dublin = round(_kpi_latest['Dublin'] / master['Monthly_Income'].dropna().iloc[-1] * 100, 1)
    kpis=[
        (f"€{_kpi_latest['Dublin']:,.0f}","Dublin Rent Q3'25",f"+{((_kpi_latest['Dublin']/_kpi_first['Dublin'])-1)*100:.0f}% since 2007","pos"),
        (f"€{_kpi_latest['Non_Dublin']:,.0f}","Non-Dublin Rent Q3'25",f"+{((_kpi_latest['Non_Dublin']/_kpi_first['Non_Dublin'])-1)*100:.0f}% since 2007","pos"),
        (f"{_afford_dublin:.1f}%","Dublin Rent-to-Income","30% = Crisis threshold","pos"),
    ...
    with c2:
        ...
        fig2.add_shape(type="line",x0=county_snapshot['Rent_Q32025'].mean(),x1=county_snapshot['Rent_Q32025'].mean(),y0=0,y1=1,xref="x",yref="paper",line=dict(color=ACCENT3,width=2,dash="dot"))
        fig2.add_annotation(x=county_snapshot['Rent_Q32025'].mean(),y=0.99,xref="x",yref="paper",text="Avg",showarrow=False,font=dict(color=ACCENT3,size=10),xanchor="left",bgcolor="rgba(0,0,0,0.4)",borderpad=2)
    ...
    with c3:
        ...
        afd=master.dropna(subset=['Affordability_Dublin'])
        fig3=go.Figure()
        ...
        fig3.add_hline(y=30,line_dash="solid",line_color=ACCENT3,line_width=2)
        fig3.add_annotation(x=0.01,y=30,xref="paper",yref="y",text="30% Crisis",showarrow=False,font=dict(color=ACCENT3,size=10),xanchor="left",yanchor="bottom")
    ...
    with c4:
        ...
        fig4.add_hline(y=4,line_dash="dash",line_color=ACCENT4)
        fig4.add_annotation(x=0.01,y=4,xref="paper",yref="y",text="RPZ 4%",showarrow=False,font=dict(color=ACCENT4,size=10),xanchor="left",yanchor="bottom")
        fig4.add_hline(y=2,line_dash="dot",line_color=ACCENT5)
        fig4.add_annotation(x=0.01,y=2,xref="paper",yref="y",text="RPZ 2%",showarrow=False,font=dict(color=ACCENT5,size=10),xanchor="left",yanchor="bottom")
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    for xd,lb,cl in [('2008-07-01','GFC 2008','#FF6B6B'),('2016-10-01','RPZ Intro','#9B59B6'),('2020-03-01','COVID-19','#F5A623')]:
        safe_vline(fig,xd,lb,cl,dash="dash",width=1.2)
    dark_layout(fig,"Average Monthly Rent by Region",height=500)
    fig.update_layout(legend=dict(orientation="h",yanchor="bottom",y=1.04,xanchor="left",x=0))
    st.plotly_chart(fig,use_container_width=True)
    ...
    with c1:
        ...
        fig2.add_hline(y=4,line_dash="dash",line_color=ACCENT4)
        fig2.add_annotation(x=0.01,y=4,xref="paper",yref="y",text="4% cap",showarrow=False,font=dict(color=ACCENT4,size=10),xanchor="left",yanchor="bottom")
        fig2.add_hline(y=2,line_dash="dot",line_color=ACCENT5)
        fig2.add_annotation(x=0.01,y=2,xref="paper",yref="y",text="2% cap",showarrow=False,font=dict(color=ACCENT5,size=10),xanchor="left",yanchor="bottom")
    ...
    with c2:
        ...
        # Use rtb directly — it has Dublin_Exist and Non_Dublin_Exist columns reliably
        ex=rtb.dropna(subset=['Dublin_Exist','Non_Dublin_Exist'])
        fig3=go.Figure()
        fig3.add_trace(go.Scatter(x=ex['Date'],y=ex['Dublin'],name='New Dublin',line=dict(color=ACCENT1,width=2),hovertemplate="New Dublin €%{y:,.0f}<extra></extra>"))
        fig3.add_trace(go.Scatter(x=ex['Date'],y=ex['Dublin_Exist'],name='Existing Dublin',line=dict(color=ACCENT1,width=2,dash='dot'),hovertemplate="Existing Dublin €%{y:,.0f}<extra></extra>"))
        fig3.add_trace(go.Scatter(x=ex['Date'],y=ex['Non_Dublin'],name='New Non-Dublin',line=dict(color=ACCENT2,width=2),hovertemplate="New Non-Dublin €%{y:,.0f}<extra></extra>"))
        fig3.add_trace(go.Scatter(x=ex['Date'],y=ex['Non_Dublin_Exist'],name='Existing Non-Dublin',line=dict(color=ACCENT2,width=2,dash='dot'),hovertemplate="Existing Non-Dublin €%{y:,.0f}<extra></extra>"))
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    fig.add_shape(type="line",x0=nav,x1=nav,y0=0,y1=1,xref="x",yref="paper",line=dict(color=ACCENT3,width=2,dash="dash"))
    fig.add_annotation(x=nav,y=0.99,xref="x",yref="paper",text=f"Avg: €{nav:,.0f}",showarrow=False,font=dict(color=ACCENT3,size=12),bgcolor="rgba(0,0,0,0.4)",borderpad=3,xanchor="left")
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    _afd_max = afd['Affordability_Dublin'].max()
    fig.add_hrect(y0=30,y1=_afd_max+5 if _afd_max>30 else 35,fillcolor="rgba(255,107,107,0.07)",line_width=0)
    fig.add_hline(y=30,line_dash="solid",line_color=ACCENT3,line_width=2.5)
    fig.add_annotation(x=0.01,y=30,xref="paper",yref="y",text="⚠ 30% Crisis Threshold",showarrow=False,font=dict(color=ACCENT3,size=13),bgcolor="rgba(0,0,0,0.4)",borderpad=3,xanchor="left",yanchor="bottom")
    safe_vline(fig,'2016-10-01',"RPZ Intro",ACCENT5)
    ...
    # Use master (full data) for income-based calculations — mf may not have income data
    _mi = master['Monthly_Income'].dropna().iloc[-1]
    _latest = master.iloc[-1]
    da=round(_latest['Dublin']/_mi*100,1)
    na=round(_latest['Non_Dublin']/_mi*100,1)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    fig.add_hline(y=48000,line_dash="dash",line_color=ACCENT2,secondary_y=False)
    fig.add_hline(y=33000,line_dash="dot",line_color=ACCENT4,secondary_y=False)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if page=="🤖 SARIMA":
        ...
        with c1:
            ...
            fig.add_hline(y=5,line_dash="dash",line_color=ACCENT3)
            fig.add_annotation(x=0.01,y=5,xref="paper",yref="y",text="5% threshold",showarrow=False,font=dict(color=ACCENT3,size=10),xanchor="left",yanchor="bottom")
